...
onMounted(async () => {
  ...
  try {
    const res = await useCustomFetch({
      url: `/vendor/shop/details/${shopUrl.value}`,
    });
    console.log("res ", res.data);
    store.shop = res.data;
    centerBannerImages.value.url = shopData.value.banner.url;
...
const classes = `bg-[${shopConfig.value.background || "#fff"}] text-[${
  shopConfig.value.background || "#121212"
}] flex flex-row justify-between shop-main gap-5 mt-5 w-[100%]`;
